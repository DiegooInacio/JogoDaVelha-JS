...
function initializeGame() {
  ...
  // Usando isso para modificar o HTML futuramente.
  updateTitle();
  boardRegions.forEach(function (element) {
    element.classList.remove("win");
    element.innerText = "";
    element.classList.add("cursor-pointer");
    // Para limpar o tabuleiro depois de uma vitória.
    element.addEventListener("click", handleBoardClick);
  });
}

function getWinsRegions() {
  const winRegions = [];
  if (
    vBoard[0][0] &&
...
function handleBoardClick(ev) {
  ...
  // "splite" serve para dividirmos uma string, transformando ela em um array
  ...
  disableRegion(span);
  const winRegions = getWinsRegions();
